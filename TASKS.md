...
#### タスク (8項目)
- [ ] プロファイル生成器実装（Lognormal分布）
- [ ] スキルシステム（58種類のスキル対応）
- [ ] 意思決定1: 消費バンドル決定
- [ ] 意思決定2: 労働市場行動（求職/受諾/辞職）
- [ ] 意思決定3: 住居選択
- [ ] 意思決定4: 金融意思決定（貯蓄/借入/投資）
- [ ] メモリ管理（過去行動の記憶）
- [ ] Function定義（find_job, modify_needs, purchase_goods等）

#### テスト (3項目)
- [ ] プロファイル生成分布検証（Lognormalチェック）
- [ ] 4つの意思決定の妥当性テスト
- [ ] メモリ管理・履歴保存テスト

**重要ポイント**:
- 論文のAppendix Eのプロンプト例を参照
- 予算制約の検証が重要
- LLMのFunction Calling形式で実装
...
**重要ポイント**:
- ProductionFunction（Phase 1実装済み）を活用
- 在庫と需要のバランス管理
- 破産条件: cash < 0 で一定期間
...
#### タスク (4項目)
- [ ] マクロ経済指標モニタリング（GDP, インフレ, 失業率, Gini）
- [ ] 累進課税システム実装（TaxationSystem統合）
- [ ] UBI配布ロジック
- [ ] 公共サービス建設判断

**重要ポイント**:
- TaxationSystem（Phase 1実装済み）を活用
- 政策決定は保守的（温度: 0.5）
- 予算制約の管理
...
#### タスク (3項目)
- [ ] Taylor rule実装（TaylorRuleクラス統合）
- [ ] 金利平滑化（ρ=0.8）
- [ ] 預金・貸出管理

**重要ポイント**:
- TaylorRule（Phase 1実装済み）を活用
- 最も保守的（温度: 0.3）
- 金利は非負制約
...
#### タスク (3項目)
- [ ] 確率的マッチングアルゴリズム
- [ ] スキルベース効率計算（calculate_effective_labor統合）
- [ ] 求人・求職リスト管理
...
**例**: programming, marketing, manufacturing, healthcare等
...
| M2 | Phase 2-3 | 全エージェント・市場動作 | +8-12日 | ⏳ **進行中** |
| M3 | Phase 4-5 | 可視化・データ完備 | +5-7日 | 待機 |
| M4 | Phase 6 | 経済現象検証完了 | +3-5日 | 待機 |
| M5 | Phase 7-8 | 最終リリース | +4-7日 | 待機 |

**現在**: M1完了、M2に着手中（Phase 2.1進行中）

**予想完了**: M1から20-38日後（Phase 0-1に7日使用済み）
...
### 現在のタスク
**Phase 2.1: HouseholdAgent実装**

推奨開始順序:
1. スキルデータ定義（Phase 5.1と並行）
2. プロファイル生成器実装
3. 4つの意思決定モジュール実装
4. Function定義
5. テスト作成
...
**最終更新**: 2025-10-14
**現在のマイルストーン**: M2 (Phase 2進行中)
**次のマイルストーン**: M2完了（全エージェント・市場動作）
